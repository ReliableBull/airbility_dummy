...
int main()
{
  ...
  */
  ...
  while (1)
  {
    ...
    if(ix != 0)
    {
      ...
      if(fabs(t_x - c_x) > 10)
      {
        //std::cout<< "diff : " << fabs(t_x - c_x) << std::endl;
  ...
  */    
        ...
        {
          double error = t_x - c_x;
          double derivative_error = (error - abs(error_last)) / 0.3;
          error_last = derivative_error;
          

          output = 1500 + 100 + (KP*error + KD*derivative_error);

          cout<<output<<": "<<error << "  right"<<endl;
          ...
          //mavlink_msg_command_long_pack(mavlink_system.sysid , mavlink_system.compid, &msg , target_system_id , target_comonetId , command , confirmation , param3 , 2000 , 0,0,0,0,0);
          mavlink_msg_command_long_pack(mavlink_system.sysid , mavlink_system.compid, &msg , target_system_id , target_comonetId , command , confirmation , param3 , output , 0,0,0,0,0);
          port->write_message(msg);        
        }        
        else
        {
          double error = c_x - t_x;
          double derivative_error = (error - error_last) / 0.3;
